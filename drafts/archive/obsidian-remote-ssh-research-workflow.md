---
title: "リモートサーバ上の研究データを Obsidian で直接管理する — Obsidian-Remote-SSH の使い方"
emoji: "🔬"
type: "tech"
...
これまで [紹介記事](https://zenn.dev/sotashimozono/articles/obsidian-remote-ssh-introduction) と [内部設計](https://zenn.dev/sotashimozono/articles/obsidian-remote-ssh-architecture) について書きました。今回は **実際にどう使ってるか** という運用面の話です。

研究者の workflow に Obsidian を組み込むうえで、`obsidian-remote-ssh` がどんな問題を解いてくれるか、自分の実例を交えて書きます。
...
- **ラップトップ (MacBook)** — 出先で書く。電車移動中に「次の実験のメモ」を書く端末
- **デスクトップ (Mac mini)** — 自宅で書く。腰据えて分析・執筆する端末

3 つの machine から **同一の vault** にアクセスしたい。けれど vault には 100 GB の実験データ (`data/`)、5 GB の plot 画像 (`plots/`)、生成された論文 PDF (`papers/`) を含めたい。これを Obsidian Sync や Syncthing で sync するのは:

- ラップトップに 105 GB を mirror できない (ストレージ的にも、転送速度的にも)
- ジョブが新しい結果を吐いた瞬間、Obsidian で見えてほしい (ファイル数千個の sync を待ちたくない)

なので `obsidian-remote-ssh` で **vault は Panza 上にだけ置く**。
...
ラップトップでも Mac mini でも、Obsidian を開いて Panza に SSH 接続するだけ。**機械側に何も置かれない**。
...
このノートは Panza 上の `vault/notes/2026-01-15-tpqmps-bench.md` に **直接書かれます**。ラップトップを閉じても、ノートは Panza に存在し続けます。実験ジョブが走ってる間にラップトップの電源が切れても問題なし。

## 具体例 2: 計算結果を Obsidian で見る

ジョブが完走すると `data/runs/2026-01-15/cv_results.csv` と `plots/2026-01-15-cv-vs-temperature.png` が書かれます。

`fs.watch` が走ってるので、Obsidian の File Explorer に **数秒以内に** 新しいファイルが現れます。ノートに `![[plots/2026-01-15-cv-vs-temperature.png]]` と書けば inline で表示されます。

ここでポイント:

- 5 GB の plot ディレクトリをラップトップに sync してない。表示時に必要な範囲だけ pull (`read_range` RPC)
...
これも全部 Panza 側のファイル読みなので、ローカルディスクが空でもまったく問題ない。

## 複数マシン同時利用 — workspace.json の罠

ラップトップで開いてる pane 構成と Mac mini で開いてる pane 構成は別であってほしい。`workspace.json` を機械間で共有してしまうと、片方の機械で window を分割した瞬間にもう片方も追従してしまう。

このプラグインは **path mapper** で `<vault>/.obsidian/workspace.json` を **per-client subtree** にリダイレクトしています。

```text
ラップトップから:  vault/.obsidian/workspace.json
                  ↓ (実ファイルは)
...
研究者の workflow で「ノートはローカル / データはリモート」 という分裂を解消できる。`vault is on the SSH host`、ローカル replica なし、Obsidian の通常の編集体験そのまま。
